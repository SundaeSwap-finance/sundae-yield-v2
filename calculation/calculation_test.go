...
func Test_PoolForEmissions(t *testing.T) {
	...
	assert.Equal(t, map[string]uint64{"C": 300, "B": 200}, selectedPools)
	...
	assert.Equal(t, map[string]uint64{"C": 202}, selectedPools)
	...
	assert.Equal(t, map[string]uint64{"F": 601, "E": 500, "D": 400, "C": 300, "B": 200}, selectedPools)
}
...
func Test_EmissionsToPools(t *testing.T) {
	...
	assert.Equal(t, map[string]uint64{"A": 500_000_000_000}, emissions)

	emissions = DistributeEmissionsToPools(program, map[string]uint64{
		"A": 1000,
		"B": 1000,
	})
	assert.Equal(t, map[string]uint64{"A": 250_000_000_000, "B": 250_000_000_000}, emissions)

	emissions = DistributeEmissionsToPools(program, map[string]uint64{
		"A": 1000,
		"B": 2000,
	})
	assert.Equal(t, map[string]uint64{"A": 166_666_666_666, "B": 333_333_333_334}, emissions)
}
...
func Test_OwnerByLPAndAsset(t *testing.T) {
	...
	assert.Equal(t, map[string]chainsync.Value{
		"A": {Assets: map[chainsync.AssetID]num.Int{"LP_X": num.Uint64(100)}},
	}, byOwner)
	assert.Equal(t, map[chainsync.AssetID]uint64{
		"LP_X": 100,
	}, byAsset)
...
func Test_RegroupByAsset(t *testing.T) {
	...
	assert.Equal(t, map[chainsync.AssetID]uint64{"LP_X": 100}, byAsset)

	byAsset = RegroupByAsset(map[string]uint64{"X": 100, "Y": 200}, pools)
	assert.Equal(t, map[chainsync.AssetID]uint64{"LP_X": 100, "LP_Y": 200}, byAsset)
}
...
func Test_EmissionsToOwners(t *testing.T) {
	...
	assert.Equal(t, map[string]uint64{"A": 1000}, emissionsByOwner)
	...
	emissionsByOwner = DistributeEmissionsToOwners(lpByOwners, emissionsByAsset, lpTokensByAsset)
	assert.Equal(t, map[string]uint64{"A": 334, "B": 666}, emissionsByOwner)
	...
	emissionsByOwner = DistributeEmissionsToOwners(lpByOwners, emissionsByAsset, lpTokensByAsset)
	assert.Equal(t, map[string]uint64{"A": 334 + 500, "B": 666}, emissionsByOwner)
}
